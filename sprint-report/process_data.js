...
function extractSprint(listPrint) {
	let targets = [];
	let sprints = [];

	for (let item of listPrint) {
		if (item.name === 'Sprint') {
...
try {
	...
	listSprint = extractSprint(projectData.fields.nodes);
	listIssue = projectData.items.nodes;
} catch (error) {
	return {
		json: {
			error: true,
...
function processSprint(listPrint, listIssue) {}
...
listIssue.forEach((item) => {
	...
	if (exists(issue.issueType.name)) {
